...
async function loadAll() {
  const steps = [
    ...
    ["readers", "./views/readers.js"],
    ["borrows", "./views/borrows.js"],
    ["logs", "./views/logs.js"],
    ["catalog", "./views/catalog.js"],
    ["myborrows", "./views/myborrows.js"],
    ["profile", "./views/profile.js"],
  ];
  for (const [name, path] of steps) {
    bootP().textContent = `加载模块 ${name}…`;
...
async function enterApp() {
  ...
  buildShell();
  // 后台预热共享数据（分类/地区/全量图书），后续进页面零请求
  api.listCategories().catch(() => {});
  api.listRegions().catch(() => {});
  api.listBooks({}).catch(() => {});
...
function wireGlobal() {
  ...
  window.openServerSetup = () => new Promise((done) => renderServerSetup(done));
}
...
async function bootstrap() {
  ...
  // Tauri/移动端瘦客户端：未配置服务器地址时，先引导配置（填 Tailscale 地址）。
  if (isTauri && !getBase()) {
    await new Promise((done) => renderServerSetup(done));
  }
