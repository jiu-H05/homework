...
export function renderLogin(onSuccess) {
  ...
  root.innerHTML = `
    <div class="login-card">
      ...
      <div id="login-pane">
        ...
        <p class="form-hint" style="margin-top:6px"><a href="javascript:void(0)" id="lnk-server" style="color:#888">服务器设置…</a></p>
      </div>
  ...
  const lnk = root.querySelector("#lnk-server");
  if (lnk) lnk.onclick = () => window.openServerSetup && window.openServerSetup();

  root.querySelector("#register-form").addEventListener("submit", async (e) => {    e.preventDefault();
    const f = e.target;
    const btn = f.querySelector('button[type="submit"]');
